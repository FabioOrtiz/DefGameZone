...
class IndexController{
    ...
    public async getIDUser(req: Request, res: Response) {
        const { id} = req.params;
        await pool.query('SELECT id FROM user WHERE nick = ?' , [id], function(err, result, fields){
            if(err) throw err;
            res.json(result);
        });
    }
    ...
    public async getIDGame(req: Request, res: Response) {
        const { id} = req.params;
        await pool.query('SELECT game_id FROM game_user WHERE user_id = ?' , [id], function(err, result, fields){
            if(err) throw err;
            res.json(result);
        });
    }
